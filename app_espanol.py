...
valores_usuario = {
    campo: st.session_state.get(campo)
    for campo in campos
}

if st.button("Calcular riesgo"):
    # Validar que todos los campos estén completos
    if None in valores_usuario.values():
        st.warning("⚠️ Por favor, llena todos los campos del formulario antes de calcular el riesgo.")
    else:
        entrada_convertida = {
        campo: valores_usuario[campo] for campo in campos
        }
        ...
        if riesgo == 1:
        st.error(f"⚠️ Riesgo alto de requerir vasopresores. Probabilidad: {probabilidad:.2f}")
        else:
        st.success(f"✅ Riesgo bajo de requerir vasopresores. Probabilidad: {probabilidad:.2f}")
